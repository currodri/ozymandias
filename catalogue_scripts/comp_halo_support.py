...
if __name__ == '__main__':
    ...
    ax.set_ylabel(r'Halo gravitational support', fontsize=16)
    ...
    for i in range(0, len(args.model)):
        ...
        if not sim.simulation.physics['cr'] and not sim.simulation.physics['magnetic']:
            ax.plot(time,ther_support, marker='s', markeredgecolor='b',markersize=4,
                    markerfacecolor='none',linestyle=':',color='b',label=args.model[i])
        elif not sim.simulation.physics['cr'] and sim.simulation.physics['magnetic']:
            ax.plot(time,ther_support, marker='s', markeredgecolor='m',markersize=4,
                    markerfacecolor='none',linestyle=':',color='m',label=args.model[i])
        elif sim.simulation.physics['cr']:
            cr_support = np.asarray(cr_support[::-1])
            ax.plot(time,cr_support+ther_support, marker='s', markeredgecolor='g',markersize=4,
                    markerfacecolor='g',label=args.model[i],linestyle='-',color='g')
            ax.plot(time,cr_support, marker='s', markeredgecolor='g', markerfacecolor='g', markersize=4,
                    linestyle='-.',color='g')
            ax.plot(time,ther_support, marker='s', markeredgecolor='g', markerfacecolor='none',markersize=4,
                    linestyle=':',color='g')
        if i==0:
    ...
    dummy_lines.append(ax.plot([],[], color="black", ls = '-',label = r'$ - \langle (\nabla P_{\rm ther} + \nabla P_{\rm cr}) /  \rho_{\rm gas}\nabla \phi \rangle $')[0])
    dummy_lines.append(ax.plot([],[], color="black", ls = '-.',label = r'$ - \langle \nabla P_{\rm cr} /  \rho_{\rm gas}\nabla \phi \rangle$')[0])
    dummy_lines.append(ax.plot([],[], color="black", ls = ':',label = r'$ - \langle \nabla P_{\rm ther} /  \rho_{\rm gas}\nabla \phi \rangle$')[0])
    second_legend = ax.legend(handles=dummy_lines, loc='lower left', frameon=False, fontsize=10)         
    ax.add_artist(second_legend)
    ax.plot([0.0, maxt],[1.0,1.0],color='k',linestyle='--')
    fig.subplots_adjust(top=0.89, bottom=0.1,right=0.97,hspace=0.0,left=0.1)
    if args.NUT:
        args.ind = 'NUT'
