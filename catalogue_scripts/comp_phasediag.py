            ...
            for i in range(0, len(args.model)):
                ...
                if args.doflows:
                    ...
                    outflow = compute_phase_diagram(gal,os.path.join(groupspath, ozyfile), 'density','temperature', [args.field],
                                [args.weight],save=True,recompute=False, filter_conds='v_sphere_r/>/10/km*s**-1', filter_name='outflow'+'_'+args.region,
                                rmin=rmin,rmax=rmax)
                    inflow = compute_phase_diagram(gal,os.path.join(groupspath, ozyfile), 'density','temperature', [args.field],
                                [args.weight],save=True,recompute=False, filter_conds='v_sphere_r/</-10/km*s**-1', filter_name='inflow'+'_'+args.region,
                                rmin=rmin,rmax=rmax)
                    escape = compute_phase_diagram(gal,os.path.join(groupspath, ozyfile), 'density','temperature', [args.field],
                                [args.weight],save=True,recompute=False, filter_conds='v_sphere_r/>=/%.3f/km*s**-1'%v_escape, filter_name='escaping'+'_'+args.region,
                                rmin=rmin,rmax=rmax)
                pd = compute_phase_diagram(gal,os.path.join(groupspath, ozyfile), 'density','temperature', [args.field],
                            [args.weight],save=True,recompute=False,rmin=rmin,rmax=rmax,filter_name=args.region)
                if args.doflows:
                    pds.append([pd,outflow,inflow,escape])
                else:
                    pds.append(pd)
                os.chdir(origfolder)
            plot_compare_phase_diagram(pds,args.field.split('/')[1],'compare_pd_'+args.region+'_'+args.field.split('/')[1]+'_'+str(args.z[z]),
