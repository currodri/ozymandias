module subbox
  ...
  real(dp),parameter      :: rel_prec = 1.e-6
  real(dp),parameter      :: maxnpart = 50000000    ! max nb of particles a sub-box may contain
  ...
  real(dp)                :: xmin_subbox,xmax_subbox,ymin_subbox,ymax_subbox,zmin_subbox,zmax_subbox 
  ! sub-box boundaries in physical Mpc (_not_ comoving)
  real(dp)                :: xmin_subbox_pt,xmax_subbox_pt,ymin_subbox_pt,ymax_subbox_pt,zmin_subbox_pt,zmax_subbox_pt
  
  ! sub-box lengths (box units)
  real(dp)                :: subbox_xc,subbox_yc,subbox_zc     ! center of the sub-box
  real(dp)                :: subbox_xl,subbox_yl,subbox_zl     ! half side of the sub-box (with its pad)
  real(dp)                :: subbox_xl2,subbox_yl2,subbox_zl2  ! half side of s-b with pad minus danger zone...
  
  real(dp)                :: padLength,padlength_pt
  real(dp)                :: dangerPadLength
...
!***********************************************************************
  subroutine select_subbox_particles_ra(n,x,vx,id,m)
    ...
    integer(kind=4)             :: n
    real(dp)                :: x(n,3),vx(n,3)
    integer(kind=4)             :: id(n)
    real(dp),optional       :: m(n)
    integer(kind=4)             :: i,np,j,ierr
    integer(kind=4),allocatable :: list(:)
    logical(kind=4),allocatable :: padlist(:)
    real(dp)                :: dx,dy,dz
...
!***********************************************************************
  ...
  subroutine read_gadget_subbox(name_file)
    ...
    real(dp),allocatable             :: Gd_pos(:,:),Gd_vel(:,:)    
    real(dp)                         :: Lbox_pt_local
    ! softening lengths (comoving, and maximum physical in kpc/h) used in the simulation:
    real(dp),parameter               :: HRepscom = 10.0, HRepsmaxph = 5.0
    real(dp),parameter               :: LRepscom = 10.0, LRepsmaxph = 5.0
    integer(kind=4)                      :: i,ierr
    real(dp)                         :: Omega0,OmegaBaryon,mass_fac,mass_box 
...
!!$    print*,'in pos:',ierr
    ...
    mass_box = io_header%massarr(2)*10./H_f*real(nbodies_full_sim,kind=dp) ! in 10^11 M_sun
    massp    = 1./real(nbodies_full_sim,kind=dp) !in units of mboxp !io_header%massarr(2)*10./H_f/mass_box  
...
!!$       end do
    ...
    print*,'+++++++++++++++++ massp from jeje +++++++++++++++++++> ',massp,real(1.0d0/dble(nbodies_full_sim),kind=dp)
