...
!             
!              where epsilon=fudgepsilon*L/Nbodies^{1/3}. Fudgepsilon is thus expressed
!              in units of mean interparticle separation.
!              - For a treecode, fudgepsilon is typically of the order of 1/20;
!              - For a PM code, fudgepsilon is typically of the order of N_g/Nbodies^{1/3}
...
!              case, the number of particles nbodies in the first line, followed by id(:), 
!              where id(:) is an array of nbodies integers.
...
!=======================================================================
  ...
  ! jeje 
  !epsilon  = fudgepsilon*xlong/dble(nbodies)**(1.d0/3.d0)
  epsilon  = fudgepsilon*xlong/dble(nbodies_full_sim)**(1.d0/3.d0)
  ! end jeje
  pos      = pos * boxsize2
...
!=======================================================================
  ...
  real(dp)    :: maxmasssub,mass_acc
  ! jeje 
  integer(kind=4) :: minid
  integer(kind=4) :: nodeMinPartID(nnodes)
  ! end jeje 
  ...
  ! jeje : record smallest particle ID of each node. 
  minid         = nbodies_full_sim + 10
  nodeMinPartID = minid
  do ip = 1,nbodies
     inode  = liste_parts(ip)
     if(inode.gt.0) then
        if (nodeMinPartID(inode) > particleID(ip)) nodeMinPartID(inode) = particleID(ip)
     end if
  end do
  if (megaverbose) write(errunit,*) '> Done recording min ID of each node ... '
  ! end jeje
  ...
  do inode = nnodes, 1, -1
     ...
     do while(isub.gt.0)
        ...
        ! jeje 
           minid = nodeMinPartID(isub)
        else if (node(isub)%truemass .eq. maxmasssub) then 
           if (nodeMinPartID(isub) .lt. minid) then 
              maxmasssub         = node(isub)%truemass
              mostmasssub(inode) = isub
              minid = nodeMinPartID(isub)
           end if
  ...
  if (megaverbose) write(errunit,*) '> Done with infinite loop ... '
...
!!$        call read_last_brick(filelast,ierr)
!!$        if(ierr.ne.0) then
!!$           write(errunit,*) '> tree_brick: ', trim(filelast),' doesn''t exist'
!!$        end if
     endif
  end if
...
!=======================================================================
  ...
  integer(kind=4) :: i
  real(dp)  :: density1,density2,rho_sad12
  logical :: exist
  logical, allocatable :: touch(:)
  integer, allocatable :: listg(:)
  real(dp),  allocatable :: rho_sad(:)  
...
!=======================================================================
  ...
  implicit none
  real(dp)          :: dist2(0:nvoisins)
  integer(kind=4)       :: iparnei(nvoisins)
  real(dp)          :: r,unsr,contrib
  real(dp),external :: spline
  integer(kind=4)       :: idist,ipar

  r=max(tiny(0e0)**(1./3.),sqrt(dist2(nvoisins))*0.5)
  unsr=1./r
  contrib=0.
  do idist=1,nvoisins-1
     if(allocated(mass)) then
        !print*,'A',idist,dist2(idist),unsr
        !print*,'B',iparnei(idist)
        !print*,'C',spline(sqrt(dist2(idist))*unsr)
        !print*,'D',mass(iparnei(idist)),unsr,spline(sqrt(dist2(idist))*unsr)
...
!=======================================================================
  ...
  real(dp)    :: poshere(3),dist2(0:nvoisins)
  real(dp)    :: dx,dy,dz,distance2,sc
  integer(kind=4) :: idist,inc
  integer(kind=4) :: icellid_out
  real(dp)    :: discell2(0:8)
  integer(kind=4) :: iparnei(nvoisins)
  integer(kind=4) :: icid(8)

  integer(kind=4) :: i,nbodies_pos_this_node
  real(dp)    :: distance2p
  ...
  do while (icell_identity.ne.0)
     ...
     dx=max(0.,min(dx,real(xlong,kind=dp)-dx)-sc)
     dy=abs(pos_cell(2,icell_identity)-poshere(2))
     dy=max(0.,min(dy,real(ylong,kind=dp)-dy)-sc)
     dz=abs(pos_cell(3,icell_identity)-poshere(3))
     dz=max(0.,min(dz,real(zlong,kind=dp)-dz)-sc)
  ...
  do ic=1,inc-1
     ...
     if (firstchild(icellid_out) < 0) then
        if (discell2(ic).lt.dist2(nvoisins)) then
           nbodies_pos_this_node=-firstchild(icellid_out)-1
           do i=nbodies_pos_this_node+1,nbodies_pos_this_node+mass_cell(icellid_out)
              iparcell=idpart(i)
              dx=abs(pos(iparcell,1)-poshere(1))
              dx=max(0.,min(dx,real(xlong,kind=dp)-dx))
              dy=abs(pos(iparcell,2)-poshere(2))
              dy=max(0.,min(dy,real(ylong,kind=dp)-dy))
              dz=abs(pos(iparcell,3)-poshere(3))
              dz=max(0.,min(dz,real(zlong,kind=dp)-dz))
...
!=======================================================================
  ...
  integer(kind=4) :: nbodies_this_node,nbodies_pos_this_node
  integer(kind=4) :: ncell
  real(dp)    :: pos_this_node(3)

  if (verbose) write(errunit,*) 'Create tree structure...'

  ! we modified to put 2*nbodies-1 instead of 2*nbodies so that AdaptaHOP can work on a 1024^3, 2*(1024^3)-1 is still an integer(kind=4), 2*(1024^3) is not 
  ncellmx=2*nbodies -1
  ncellbuffer=max(nint(0.1*nbodies),ncellbuffermin)
  allocate(idpart(nbodies))
  allocate(idpart_tmp(nbodies))
  ...
  idmother=0
  pos_this_node(1:3)=0.
  nbodies_this_node=nbodies
  nbodies_pos_this_node=0
  idpart_tmp(1:nbodies)=0
 ...
 &                   nbodies_this_node,nbodies_pos_this_node,inccell, &
 &                   idmother)
  ncell=inccell
...
!=======================================================================
recursive subroutine create_KDtree(nlevel,pos_this_node,nbodies_this_node, &
 &                nbodies_pos_this_node,inccell,idmother)
!=======================================================================
!  nlevel : level of the node in the octree. Level zero corresponds to 
!           the full box
!  pos_this_node : position of the center of this node
!  nbodies  : total number of particles 
!  idpart : array of dimension nbodies containing the id of each
...
!           idpart.
!  nbodies_this_node : number of particles in the considered node
!  nbodies_pos_this_node : first position in idpart of the particles 
!           belonging to this node
!  pos :    array of dimension 3.nbodies giving the positions of 
!           each particle belonging to the halo
!  inccell : cell id number for the newly created structured grid site
!  pos_cell : array of dimension 3.nbodies (at most) corresponding
!           to the positions of each node of the structured grid
!  mass_cell : array of dimension nbodies (at most) giving the 
!           number of particles in each node of the structured grid
!  size_cell : array of dimension nbodies (at most) giving half the
...
!=======================================================================
   ...
   integer(kind=4)           :: nlevel,nbodies_pos_this_node,nbodies_this_node
   real(dp)              :: pos_ref(3,0:7)
   real(dp)              :: pos_this_node(3)   
   integer(kind=4)           :: ipar,icid,j,inccell,nlevel_out
   integer(kind=4), external :: icellid
   integer(kind=4)           :: nbodies_pos_this_node_out,nbodies_this_node_out
   integer(kind=4)           :: incsubcell(0:7),nsubcell(0:7)
   real(dp)              :: xtest(3),pos_this_node_out(3)
   ...
   real(dp), allocatable :: size_cell_tmp(:),pos_cell_tmp(:,:)
...
!           node.
 ...
 &                 1., 1., 1.  / 
   ...
   if (nbodies_this_node.gt.0) then
      inccell=inccell+1
 ...
 &          'ncellmx is too small. Increase it and reallocate arrays accordingly'
      ...
      mass_cell(inccell)=nbodies_this_node
...
!     maximum level of refinement, we are done
      if ((nbodies_this_node <= nbodiespercell).or.(nlevel.eq.nlevelmax)) then
         firstchild(inccell)=-(nbodies_pos_this_node+1)
         return
      endif
   else
      return
...
!  Count the number of particles in each subcell of this node
   incsubcell(0:7)=0
   do ipar=nbodies_pos_this_node+1,nbodies_pos_this_node+nbodies_this_node
...
!  to the same subnode. Put the result in idpart_tmp.
   ...
   do ipar=nbodies_pos_this_node+1,nbodies_pos_this_node+nbodies_this_node
      ...
      icid=icellid(xtest)
      incsubcell(icid)=incsubcell(icid)+1
      idpart_tmp(incsubcell(icid)+nsubcell(icid)+nbodies_pos_this_node)=idpart(ipar)
   enddo
...
!  and call for the treatment recursively.
 ...
 &                           sizeroot*pos_ref(1:3,j)*2.**(-nlevel-2)
      nbodies_pos_this_node_out=nbodies_pos_this_node+nsubcell(j)
      nbodies_this_node_out=incsubcell(j)
      call create_KDtree(nlevel_out,pos_this_node_out,nbodies_this_node_out, &
 &                nbodies_pos_this_node_out,inccell,idmother_out)
   enddo
...
!=======================================================================
  ...
  implicit none
  real(dp), parameter :: accurac=1.e-6

  integer, allocatable, dimension(:) :: idgene
  real(dp),allocatable, dimension(:) :: tmp
  real(dp),allocatable, dimension(:,:) :: possav
  logical,allocatable,dimension(:) :: move
  real(dp) :: xref,tolerance,tolerance2,phi,costeta,teta,sinteta
  real(dp) :: ran2
  ...
  allocate(idgene(nbodies),tmp(nbodies),move(nbodies))
  tolerance2=2.0*tolerance
  allocate(possav(3,nbodies))
  do i=1,nbodies
  ...
  do while (.not.doneiter) 
     ...
     do i=1,nbodies
        ...
        move(i)=.false.
     enddo
     call indexx(nbodies,tmp,idgene)
