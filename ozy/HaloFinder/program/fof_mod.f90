module fof
  ...
  ! octtree
  type cell
     real(dp)    :: size                    ! half lengh of the cell
     real(dp)    :: pos(3)                  ! position of the center of the cell
  ...
  real(dp)    :: pos_sh(3,8), p0(6,3), sizemax, sizemin
  integer(kind=4), allocatable :: cellmother(:),cellsister(:),cellchild(:)
  real(dp),    allocatable :: cellpos(:,:), cellsize(:)
...
! case if omega_baryons is not zero!
  if (allocated(mass)) then 
     ...
     mass_eq = 1d0/real(nbodies_full_sim,8)
     ! end jeje 
  endif
  fEps      = b_init*real(mass_eq,kind=dp)**(1./3.)
  fEps2     = fEps*fEps
