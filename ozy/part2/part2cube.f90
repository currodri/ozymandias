subroutine getcube(repository,ageweight,periodic,star,xmin,xmax,ymin,ymax,zmin,zmax,&
      & toto,totsum,nx,ny,nz)
  !--------------------------------------------------------------------------
  ...
  implicit none
	...
	!Output variables
	real(KIND=4),dimension(nx+1,ny+1,nz+1)::toto
	real::totsum

!f2py intent(in) repository,ageweight,periodic,star,xmin,xmax,ymin,ymax,zmin,zmax,nx,ny,nz
...
!f2py intent(out) totsum
  ...
  integer::ix,iy,iz,ixp1,iyp1,izp1,idim,jdim,kdim,ncpu_read
  ...
  character(LEN=128)::nomfich,outfich
  logical::ok,ok_part
...
subroutine integrate(repository,ageweight,periodic,star,xmin,xmax,ymin,ymax,zmin,zmax,&
      & totsum, totpart)
  !--------------------------------------------------------------------------
  ! Ce programme calcule la carte de densite surfacique projetee
  ! des particules de matiere noire d'une simulation RAMSES. 
...
!f2py intent(out) totsum,totpart
