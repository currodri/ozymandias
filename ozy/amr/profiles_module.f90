...
module amr_profiles
    ...
    subroutine findbinpos_twod(reg,distance,pos,cellvars,cellsons,cellsize,prof,ibinx,ibiny,trans_matrix,grav_var)
        use vectors
        use geometrical_regions
        implicit none
        ...
        ! print*,value
        if (trim(prof%scaletype(1)).eq.'log_even') then
        ...
        ! print*,value
        if (trim(prof%scaletype(2)).eq.'log_even') then
            if (value.le.0D0) then
                    ibiny = 0
            else
    ...
    subroutine bindata(reg,pos,cellvars,cellsons,cellsize,prof,ibin,ifilt,trans_matrix,grav_var)
        ...
        ! Local variables
        integer :: i,j,ipdf
        real(dbl) :: ytemp,ytemp2,wtemp,bigwtemp,bigatemp
        type(vector) :: x
        x = pos
        ...
        yvarloop: do i=1,prof%ydata(ibin)%nvars
            if (prof%ydata(ibin)%do_binning(i)) then
                ...
                ! Get variable
                if (present(grav_var)) then
                    call getvarvalue(reg,cellsize,x,cellvars,cellsons,prof%ydata(ibin)%varname(i),ytemp,trans_matrix,grav_var)
                else
                    call getvarvalue(reg,cellsize,x,cellvars,cellsons,prof%ydata(ibin)%varname(i),ytemp,trans_matrix)
                end if
                ! Get min and max
                if (prof%ydata(ibin)%minv(i,ifilt).eq.0D0) then
                    prof%ydata(ibin)%minv(i,ifilt) = ytemp ! Just to make sure that the initial min is not zero
    ...
    subroutine onedprofile(repository,reg,prof_data,lmax,use_neigh,read_gravity)
        use geometrical_regions
        implicit none
        ...
        subroutine get_cells_onedprofile_neigh
            ...
            ! logical :: filter_cell
            integer :: i,j,k
            integer :: ipos,icpu,ilevel,ind,idim,ivar,iskip,inbor,ison,isub,ifilt
    ...
    subroutine twodprofile(repository,reg,prof_data,lmax)
        use geometrical_regions
        implicit none
        character(128),intent(in) :: repository
        type(region),intent(inout) :: reg
        ...
        call get_cpu_map(reg)
        if (verbose) write(*,*)'ncpu_read:',amr%ncpu_read
        call get_cells_twodprofile(repository,reg,prof_data)
        call renormalise_bins_twod(prof_data)

    end subroutine twodprofile

    subroutine get_cells_twodprofile(repository,reg,prof_data)
        use vectors
        use coordinate_systems
        use geometrical_regions
        implicit none
                                    ...
                                    if (ok_filter) then
                                        ...
                                        if (read_gravity) then
                                            call findbinpos(reg,xtemp,tempvar,tempson,&
                                                            ...
                                                            &prof_data%xvarname,tempgrav_var)
                                            call findbinpos(reg,xtemp,tempvar,tempson,&
